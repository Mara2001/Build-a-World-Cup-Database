...
echo "$($PSQL "SELECT team_name FROM (SELECT DISTINCT t1.name AS team_name FROM games INNER JOIN teams AS t1 ON games.winner_id = t1.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT DISTINCT t2.name AS team_name FROM games INNER JOIN teams AS t2 ON games.opponent_id = t2.team_id WHERE year = 2014 AND round = 'Eighth-Final' ) AS combined_teams ORDER BY team_name;")"
